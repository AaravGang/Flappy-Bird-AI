...
for n in range(1, NUM_BIRD_IMAGES + 1):
    ...
    BIRD_IMAGES.append(
        pygame.transform.scale(image, (BIRD_DIMENSIONS, BIRD_DIMENSIONS))
    )
    BIRD_MASKS.append(
...
PIPE_IMG = pygame.transform.scale(
    pygame.image.load("images/" + "pipe.png"), (PIPE_WIDTH, PIPE_HEIGHT)
)
pipe_top_mask = pygame.mask.from_surface(
...
class Pipe:
    ...
    def __init__(self, x=WIDTH):
        ...
        self.top, self.bottom = (
            0,
            0,
        )  # initialise the top and bottom cors of the pipe to 0
    ...
    def collide(self, bird, pixel_perfect=False):
        if not pixel_perfect:
            ...
            top_offset = (
                self.x - bird.x,
                self.top - self.TOP_IMG.get_height() - round(bird.y),
            )
